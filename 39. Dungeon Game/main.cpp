...
public:
	int calculateMinimumHP(vector<vector<int>>& dungeon){
		for (int i=0; i<dungeon[0].size(); i++){
			for (int j=0; j<dungeon.size(); j++){
				reverse_calculation(dungeon, dungeon[0].size()-1-i, dungeon.size()-1-j);
				//cout<<dungeon[dungeon.size()-1-j][dungeon[0].size()-1-i]<<endl;
			}
		}
		return 1+dungeon[0][0];
		//return calculateMinimumHP(dungeon, 0, 0, 1, 0);//Первый вариант
	}
	int calculateMinimumHP(vector<vector<int>>& dungeon, int knight_x_pos, int knight_y_pos, int minimum_initial_hp/*сколько должно было быть хп, чтобы не умереть*/, int cur_hp_bonus/*оверхил*/){
		//cout<<"knight_x_pos "<<knight_x_pos<<" knight_y_pos "<<knight_y_pos<<endl;
		//cout<<"Branching "<< (knight_x_pos==dungeon[0].size()-1) << (knight_y_pos==dungeon.size()-1) <<endl;
		//if (minimum_initial_hp_recorded < minimum_initial_hp) return minimum_initial_hp;
		
		if (knight_x_pos==dungeon[0].size()-1){
			//Только движение вниз
			if (knight_y_pos==dungeon.size()-1){
				//Достигли принцессы
				if (cur_hp_bonus +  dungeon[knight_y_pos][knight_x_pos] >= 0){
					cur_hp_bonus += dungeon[knight_y_pos][knight_x_pos];
				} else {
					minimum_initial_hp -= dungeon[knight_y_pos][knight_x_pos] + cur_hp_bonus;
					cur_hp_bonus = 0;
				}
				//minimum_initial_hp_recorded = minimum_initial_hp;
				return minimum_initial_hp;
			} else {
				if (cur_hp_bonus +  dungeon[knight_y_pos][knight_x_pos] >= 0){
					cur_hp_bonus += dungeon[knight_y_pos][knight_x_pos];
				} else {
				...
				return calculateMinimumHP(dungeon, knight_x_pos, knight_y_pos+1, minimum_initial_hp, cur_hp_bonus);
				//Движение вниз
			}
		} else {
			//Доступно движение вправо
			if (knight_y_pos==dungeon.size()-1){
				//Только вправо
				//cout<<"Again: knight_x_pos "<<knight_x_pos<<" knight_y_pos "<<knight_y_pos<<endl;
				//cout<<"OK? "<< cur_hp_bonus << endl << dungeon[knight_y_pos][knight_x_pos] <<endl;
				if (cur_hp_bonus +  dungeon[knight_y_pos][knight_x_pos] >= 0){
					cur_hp_bonus += dungeon[knight_y_pos][knight_x_pos];
				} else {
					minimum_initial_hp -= dungeon[knight_y_pos][knight_x_pos] + cur_hp_bonus;
				...
				return calculateMinimumHP(dungeon, knight_x_pos+1, knight_y_pos, minimum_initial_hp, cur_hp_bonus);
			} else {
				...
				int go_down_minimum_initial_hp, go_right_minimum_initial_hp;
				go_down_minimum_initial_hp  = calculateMinimumHP(dungeon, knight_x_pos, knight_y_pos+1, minimum_initial_hp, cur_hp_bonus);
				go_right_minimum_initial_hp = calculateMinimumHP(dungeon, knight_x_pos+1, knight_y_pos, minimum_initial_hp, cur_hp_bonus);
				return (go_down_minimum_initial_hp > go_right_minimum_initial_hp ? go_right_minimum_initial_hp : go_down_minimum_initial_hp);
			}
		}
		
		
		/*if (knight_y_pos==dungeon.size()-1){
			//Остаётся только движение вправо
			int answer = 0;
			for (int i=knight_x_pos; i<dungeon[0].size(); i++){
				if (dungeon[knight_y_pos][knight_x_pos]+minimum_initial_hp<1){
					//Он же умрёт! Значит, надо увеличить начальное хп
					minimum_initial_hp = dungeon[knight_y_pos][knight_x_pos]+minimum_initial_hp<1
				} else {
					
				}
				
				answer+=dungeon[knight_y_pos][knight_x_pos];
			}
			return answer;
		} else {
	...
	void reverse_calculation(vector<vector<int>>& dungeon, int x_pos, int y_pos){
		//Второй вариант
		//Переделываем поэлементно матрицу dungeon под минимальное хп, необходимое для достижения точки
		//Начинаем с правого нижнего угла
		if (x_pos == dungeon[0].size() - 1){
			if (y_pos == dungeon.size() - 1){
				//Правый нижний угол
				if (dungeon[y_pos][x_pos] < 0){
					dungeon[y_pos][x_pos] *= -1;
				} else {
					dungeon[y_pos][x_pos] = 0;
				}
			} else {
				//Идём по правой стенке
				dungeon[y_pos][x_pos] = dungeon[y_pos+1][x_pos] - dungeon[y_pos][x_pos];
				if (dungeon[y_pos][x_pos] < 0){
					dungeon[y_pos][x_pos] = 0;
				}
			}
			...
			if (y_pos == dungeon.size() - 1){
				//Идём по нижней стенке
				dungeon[y_pos][x_pos] = dungeon[y_pos][x_pos+1] - dungeon[y_pos][x_pos];
				if (dungeon[y_pos][x_pos] < 0){
					dungeon[y_pos][x_pos] = 0;
				}
			} else {
				int temp_1 = dungeon[y_pos+1][x_pos] - dungeon[y_pos][x_pos];
				if (temp_1 < 0) temp_1=0;
				int temp_2 = dungeon[y_pos][x_pos+1] - dungeon[y_pos][x_pos];
				if (temp_2 < 0) temp_2=0;
				dungeon[y_pos][x_pos] = min(temp_1, temp_2);
			}
		}
	}
...
private:
	//int minimum_initial_hp_recorded = INT_MAX;
};
...
int main() {
	
	{
		...
		int answer = 6;
		auto real_answer = Solution().calculateMinimumHP(dungeon);
...
Some of the rooms are guarded by demons, so the knight loses health (negative integers) upon entering these rooms; other rooms are either empty (0's) or contain
